...
// --- llms.txt: the AI-crawler map of the site (llmstxt.org) ---
// Generated by src/pages/llms.txt.ts, so it is asserted on the built output.
assert('llms.txt built', exists('llms.txt'));
const llmsSource = exists('llms.txt') ? read('llms.txt') : '';
assert(
...
assert('llms.txt opens with the venue H1 and a summary blockquote', /^# SiSi Wrocław\n\n> \S/.test(llmsSource));
assert('llms.txt links the Polish pages, the site primary language', llmsSource.includes(`](${CANONICAL_ORIGIN}/pl/`));
assert('llms.txt has no relative or non-https links', !/\]\((?!https:\/\/)/.test(llmsSource));
assert('llms.txt leaks no {tokens}', !/\{[a-zA-Z]+\}/.test(llmsSource));
assert(
  'llms.txt points at the sitemap and both public sister profiles',
  llmsSource.includes(`${CANONICAL_ORIGIN}/sitemap.xml`)
    && llmsSource.includes('https://www.r32.com.pl/')
    && llmsSource.includes(FACEBOOK_URL),
);
assert(
